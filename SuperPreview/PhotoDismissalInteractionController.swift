...
class PhotoDismissalInteractionController: NSObject {
    ...
    private func backgroundAlphaForPanningWithVerticalDelta(_ verticalDelta: CGFloat) -> CGFloat {
        guard let fromView = transitionContext?.view(forKey: UITransitionContextViewKey.from) else {
            return 1
        }
    ...
    private func finishPanWithPanGestureRecognizer(_ pan: UIPanGestureRecognizer, verticalDelta: CGFloat, viewToPan: UIView, anchorPoint: CGPoint) {
        guard let fromView = transitionContext?.view(forKey: UITransitionContextViewKey.from) else {
            return
        }
